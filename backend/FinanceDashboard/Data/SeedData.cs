    ...
    public static class SeedData
    {
        public static async Task InitializeAsync(AppDbContext context)
        {
            if (!await context.Categories.AnyAsync())
            {
                ...
                {
                    new Transaction { Merchant = "Walmart", Amount = 87.43m, Date = DateTime.Now.AddDays(-30), CategoryId = categories[2].Id },
                    new Transaction { Merchant = "Walmart", Amount = 112.67m, Date = DateTime.Now.AddDays(-16), CategoryId = categories[2].Id },
                    new Transaction { Merchant = "Whole Foods", Amount = 54.21m, Date = DateTime.Now.AddDays(-25), CategoryId = categories[2].Id },
                    new Transaction { Merchant = "Work", Amount = 2400.00m, Date = DateTime.Now.AddDays(-28), CategoryId = categories[0].Id },
                    new Transaction { Merchant = "Work", Amount = 2400.00m, Date = DateTime.Now.AddDays(-14), CategoryId = categories[0].Id },
                    new Transaction { Merchant = "McDonalds", Amount = 12.87m, Date = DateTime.Now.AddDays(-22), CategoryId = categories[3].Id },
                    new Transaction { Merchant = "Chipotle", Amount = 18.43m, Date = DateTime.Now.AddDays(-10), CategoryId = categories[3].Id },
                    new Transaction { Merchant = "Starbucks", Amount = 6.75m, Date = DateTime.Now.AddDays(-5), CategoryId = categories[3].Id },
                    new Transaction { Merchant = "Amazon", Amount = 43.99m, Date = DateTime.Now.AddDays(-18), CategoryId = categories[4].Id },
                    new Transaction { Merchant = "Target", Amount = 67.32m, Date = DateTime.Now.AddDays(-8), CategoryId = categories[4].Id },
                    new Transaction { Merchant = "Venmo", Amount = 50.00m, Date = DateTime.Now.AddDays(-3), CategoryId = categories[1].Id },
            ...
            // Backfill any existing transactions with null CategoryId that match a rule
            var uncategorized = await context.Transactions
                .Where(t => t.CategoryId == null)
                .ToListAsync();

            if (uncategorized.Count > 0)
            {
                var allRules = await context.Rules.ToListAsync();
                foreach (var t in uncategorized)
                {
                    var match = allRules.FirstOrDefault(r => r.Merchant.Equals(t.Merchant, StringComparison.OrdinalIgnoreCase));
                    if (match != null) t.CategoryId = match.CategoryId;
                }
                await context.SaveChangesAsync();
            }
        }
